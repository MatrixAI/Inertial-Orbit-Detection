final float orbitRPSScalingFactor = 0.6;

final int balloonSize = 25;
final int basketSize = 25;

final float gravity = 1;
final float airFriction = 0.0001;
final float balloonVertVelocity = 0;
final float balloonHoriVelocity = 1;

final int wallSpeed = 3;
final int wallInterval = 1600;
final int wallWidth = 80;
final float wallLastAddTime = 0;
final color wallColors = color(240, 248, 255);
final int minGapHeight = 250;
final int maxGapHeight = 300;

final long pingPongTimeout = 5;
